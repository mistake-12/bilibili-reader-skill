...
| # | 问题分类 | 问题描述 | 解决方案 | 优先级 |
|---|---------|---------|---------|--------|
| 1 | 代码设计 | 意图路由依赖LLM判断，脆弱且主观 | 引入多维度特征提取 + 置信度评分机制 | P1 | 不引入 逻辑改成假如这视频属于多个分类的话就调用两个分类或者两个分类的内容放在一起
| 2 | 代码设计 | 字幕分段无语义感知，可能截断完整段落 | 语义分段（句子边界检测 + 段落聚类） | P2 | 不引入
| 3 | 代码设计 | 无LLM模式下总结几乎无价值 | 接入轻量级本地模型（gemma-2b / Qwen-0.5B）或提供更好的规则降级 | P1 | 不引入，作为提醒
| 4 | 代码设计 | PDF模板字段过多，样式与内容耦合 | 重构为组件化PDF模板，引入结构化渲染层 | P2 | 这个你准备怎么做呢
| 5 | 代码设计 | 记忆系统搜索仅字符串匹配，无向量检索 | 引入轻量向量数据库（ChromaDB）实现语义搜索 | P1 | 可以引入，这会有什么效果
| 6 | 代码设计 | Cookie认证脆弱，过期无感知 | 添加Cookie健康检查 + 自动刷新机制 + 过期告警 | P1 | 引入
| 7 | 代码设计 | 成就系统仅为打印，无状态存储与展示 | 持久化成就状态 + Web/终端徽章展示页面 | P3 | 不引入
| 8 | 需求出发 | 静态PDF非最优交付形式 | 从"PDF工厂"转型为"可查询知识库" | P0 | 不引入
| 9 | 需求出发 | 无用户反馈闭环，无法评估总结质量 | 引入用户评分/追问机制，形成优化飞轮 | P1 |  不引入
| 10 | 需求出发 | 体裁专用提示词人力维护成本高 | 统一提示词基座 + 体裁增强层（体裁作为元标签而非独立模板） | P2 | 这个你准备怎么做呢
| 11 | 需求出发 | 随机选视频不符合学习规律 | 增加"学习路径推荐"功能，基于体裁多样性和知识缺口分析 | P0 | 这个你准备怎么做呢
| 12 | 需求出发 | 无真正的学习效果验证 | 增加"理解度测验"模块，基于总结内容生成测验题 | P0 | 引入，提示词已有
| 13 | 需求出发 | 缺乏数据收集，无产品迭代依据 | 埋点收集使用数据（处理量、完播率、搜索行为等） | P2 |  不引入
